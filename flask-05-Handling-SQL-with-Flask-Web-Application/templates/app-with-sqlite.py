...
app = Flask(__name__)
app.config[‘SQLALCHEMY_DATABASE_URI’] = ‘sqlite:///./email.db’ #env degiskenlerini configure!
app.config[‘SQLALCHEMY_TRACK_MODIFICATIONS’] = False
db = SQLAlchemy(app)
drop_table = ‘DROP TABLE IF EXISTS users;’ # user tablolari olusturuyoruz. ilk basta user tablosunu olusturmak icin daha sonra calismaycak.
users_table = “”"
CREATE TABLE users(
...
“”"
db.session.execute(drop_table)    #bu orderlari calistirmak icin kullaniyoruz.
db.session.execute(users_table)
db.session.execute(data)
db.session.commit()
def find_email(keyword):   # email bulma fonksiyonu
    query = f”“”
    ...
    return user_emails
def insert_email(name,email):      # email add fonksiyonlari tanimliyoruz
    query = f”“”
    ...
    return response
@app.route(‘/’, methods = [‘POST’, ‘GET’])  #email olusturmak icin dekoretorlerimiz
def emails():
...
@app.route(‘/add’, methods=[‘GET’, ‘POST’]) #add email
def add_email():
    if request.method == ‘POST’:
